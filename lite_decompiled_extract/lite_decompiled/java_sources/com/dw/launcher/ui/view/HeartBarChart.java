...
public class HeartBarChart extends View {
    ...
    public HeartBarChart(Context context) {
        ...
        this.viewWidth = 0;
        this.viewHeight = 0;
        this.max_value = PduPart.P_CONTENT_TRANSFER_ENCODING;
        this.min_value = 0;
        this.bottomTextList = new ArrayList();
        this.potLists = new ArrayList();
        this.positionIndexList = new ArrayList();
        this.slideTextList = new ArrayList(Arrays.asList(0, 50, 100, 150, Integer.valueOf(PduPart.P_CONTENT_TRANSFER_ENCODING)));
        this.startSlideTextPosition = 0;
        this.maxHeight = 0;
        this.start0Height = 0;
        this.start0WidthPosition = 0;
        this.end0WidthPostion = 0;
        this.one_step_duration = 25;
        this.per_bar_value = 50;
        ...
        this.isFinish = true;
        this.handler = new Handler(Looper.getMainLooper()) { // from class: com.dw.launcher.ui.view.HeartBarChart.1
            @Override // android.os.Handler
            public void dispatchMessage(Message msg) {
                super.dispatchMessage(msg);
                if (msg.what == 0) {
                    HeartBarChart.this.childBarAnimal();
                }
            }
        };
    ...
    public HeartBarChart(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.offset = 10;
        this.barNumbers = 24;
        this.viewWidth = 0;
    ...
    void init(AttributeSet attrs) {
        TypedArray typedArrayObtainStyledAttributes = getContext().obtainStyledAttributes(attrs, R.styleable.barChar);
    ...
    private void drawBars(Canvas canvas) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            ...
            float fIntValue = (this.end0WidthPostion - ((23 - this.positionIndexList.get(i3).intValue()) * i)) - (this.mBottomTextPaint.measureText("00") / 2.0f);
            float f = this.start0Height;
            int i4 = this.min_value;
            pointArr[i3] = new Point((int) fIntValue, (int) (f - (((value - i4) / (this.max_value - i4)) * i2)));
        }
        int iDp2px = dp2px(getContext(), 3.0f);
        for (int i5 = 0; i5 < size; i5++) {
            Point point = pointArr[i5];
            RectF rectF = new RectF(point.x - iDp2px, point.y, point.x + iDp2px, this.start0Height);
            Path path = new Path();
    ...
    private void drawSlideLine(Canvas canvas) {
        ...
        for (int i2 = 0; i2 < this.slideTextList.size(); i2++) {
            ...
            if (i2 != 0) {
                canvas.drawText(iIntValue + "", this.startSlideTextPosition, i4 + (rect.height() / 2), this.mSlideTextPaint);
            }
        }
    }
    ...
    private void drawBottomText(Canvas canvas) {
        ...
        Rect rect = new Rect();
        this.mBottomTextPaint.getTextBounds("00", 0, 2, rect);
    ...
    public void startAnimate2() {
        ...
        this.tempPosList = arrayList;
        arrayList.addAll(this.positionIndexList);
        this.tempList.addAll(this.potLists);
        this.positionIndexList = new ArrayList(Collections.nCopies(24, 0));
        this.potLists = new ArrayList(Collections.nCopies(24, 0));
        this.currentBarIndex = 0;
    ...
    int findLastFullMaxValueIndex(List<Integer> tempList) {
        for (int size = tempList.size() - 1; size >= 0; size--) {
            if (tempList.get(size).intValue() == this.max_value) {
                return size;
            }
        }
        return -1;
    ...
    int findFirstZeroValueIndex(List<Integer> tempList) {
    ...
    public void setDailyList(List valueList, List posIndexList) {
        if (this.isFinish) {
            this.potLists = valueList;
