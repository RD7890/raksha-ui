...
public class BloodPressChart extends View {
    ...
    public BloodPressChart(Context context) {
        super(context);
    ...
    public BloodPressChart(Context context, AttributeSet attributeSet) {
    ...
    private void calcPointValue() {
        ...
        this.minpoints = new Point[size];
        for (int i = 0; i < size; i++) {
            float fIntValue = this.potLists.get(i).intValue();
            int iIntValue = this.minList.get(i).intValue();
            ...
            int i5 = (int) potPostionbyIndex;
            this.points[i] = new Point(i5, (int) (f - (((fIntValue - i3) / (i4 - i3)) * f2)));
            this.minpoints[i] = new Point(i5, (int) (f - (((iIntValue - i3) / (i4 - i3)) * f2)));
        }
    }
    ...
    private void drawSlideText(Canvas canvas) {
        ...
        this.MAXVALUE = 180;
        int[] iArr = {60, 90, 120, ImsPhoneCallTrackerEx.CODE_LOCAL_CALL_CS_EMERGENCY_RETRY_REQUIRED, 180};
        ...
        for (int i3 = 0; i3 < 5; i3++) {
            ...
            canvas.drawText(iArr[i3] + BuildConfig.FLAVOR, paddingLeft, f5 + (this.bottomTextHeihtRect.height() / 2), this.mSlideLinePaint);
        }
        canvas.drawPath(path, this.mDashPaint);
    }

    private void drawScrollLine(Canvas canvas) {
        drawBar(canvas, this.points, this.minpoints);
    }

    private void drawBar(Canvas canvas, Point[] pointArr, Point[] pointArr2) {
        new Point();
        if (pointArr.length == 1) {
            Point point = pointArr[0];
            drawBars(canvas, point.x, point.y, pointArr2[0].y);
        } else {
            for (int i = 0; i < pointArr.length; i++) {
                drawBars(canvas, pointArr[i].x, pointArr[i].y, pointArr2[i].y);
            }
        }
    }

    void drawBars(Canvas canvas, float f, float f2, float f3) {
        this.mPathBgPaint.setStyle(Paint.Style.FILL);
        ...
        float f4 = this.circleRadus;
        RectF rectF = new RectF(f - f4, f2, f4 + f, f3);
        Path path = new Path();
        path.addRoundRect(rectF, new float[]{30.0f, 30.0f, 30.0f, 30.0f, 30.0f, 30.0f, 30.0f, 30.0f}, Path.Direction.CW);
        canvas.drawPath(path, this.mPathBgPaint);
        Log.d("bar", "drawBars: " + f + " " + f2 + " " + f3);
    }
